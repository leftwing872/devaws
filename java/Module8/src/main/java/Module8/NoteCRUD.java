...
public class NoteCRUD {
    public static void main(String[] args) {
        DynamoDbClient ddb = DynamoDbClient.builder()
                //.credentialsProvider(ProfileCredentialsProvider.create("profile2"))
    ...
    public static Note retrieveNote(DynamoDbEnhancedClient enhancedClient) {
        ...
        try{
            ...
            System.out.println("****** The note reads (search): " + result.getNote());
        } catch (DynamoDbException e) {
            System.err.println(e.getMessage());
        }
        return result;
    ...
    public static void addNote42(DynamoDbEnhancedClient enhancedClient) {
        try {
            DynamoDbTable table = enhancedClient.table("Notes", TableSchema.fromBean(Note.class));

            Note noteToAdd = new Note();
            noteToAdd.setUserId("StudentD");
            noteToAdd.setNoteId(42);
            noteToAdd.setFavorite("Yes");
            noteToAdd.setNote("This is a note written by JAVA SDK.");
